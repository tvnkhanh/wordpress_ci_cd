...
class shopgrove_Admin_Info {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_dismiss_shopgrove_notification', array( $this, 'dismiss_notification_ajax_handler' ) );
    }
    ...
    public function add_menu() {
        add_theme_page(
            esc_html__('ShopGrove', 'shopgrove'),
            esc_html__('ShopGrove', 'shopgrove'),
            'edit_theme_options',
            'shopgrove',
    ...
    public function reset_notification_dismissal() {
        delete_transient('shopgrove_notification_dismissed');
    }
    ...
    public function compatible_check() {
        $dismissed = get_transient('shopgrove_notification_dismissed');
        if ($dismissed !== '1') {
    ...
    public function dismiss_notification() {
        set_transient('shopgrove_notification_dismissed', '1', 7 * DAY_IN_SECONDS);
    }
            ...
            (function($) {
                $(document).on('click', '.notice.is-dismissible .notice-dismiss', function() {
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'dismiss_shopgrove_notification',
